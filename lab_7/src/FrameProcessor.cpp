...
void FrameProcessor::setFaceDetector(std::shared_ptr<FaceDetector> detector) {
...
cv::Mat FrameProcessor::process(const cv::Mat& input, ProcessingMode currentMode, const MouseState& mouseState) {
    ...
    if (faceDetector) {
        faceDetector->setFaceDetectionEnabled(mode == ProcessingMode::FACE_DETECT_MODE);
        
        if (mode == ProcessingMode::FACE_DETECT_MODE) {
            faceDetector->updateFrame(input);
        }
    }
    ...
    switch(mode) {
        ...
        case ProcessingMode::ROTATE_MODE:
            ...
            break;
        case ProcessingMode::FACE_DETECT_MODE:
            result = applyFaceDetection(input);
            break;
        default:
            result = input.clone();
    }
...
cv::Mat FrameProcessor::applyFaceDetection(const cv::Mat& input) {
    cv::Mat result = input.clone();
    
    if (faceDetector && faceDetector->isDetectionEnabled()) {
        std::vector<Face> faces = faceDetector->getFaces();
        
        for (const auto& face : faces) {
            if (face.isValid) {
                cv::rectangle(result, face.boundingBox, cv::Scalar(0, 255, 0), 2);
                
                std::string confidence = std::to_string(static_cast<int>(face.confidence * 100)) + "%";
                cv::putText(result, confidence, 
                           cv::Point(face.boundingBox.x, face.boundingBox.y - 5),
                           cv::FONT_HERSHEY_SIMPLEX, 0.5, cv::Scalar(0, 255, 0), 1);
            }
        }
        
        if (faces.empty()) {
            cv::putText(result, "No face detected", cv::Point(10, 120),
                       cv::FONT_HERSHEY_SIMPLEX, 0.6, cv::Scalar(0, 0, 255), 2);
        } else {
            cv::putText(result, "Faces detected: " + std::to_string(faces.size()), 
                       cv::Point(10, 120), cv::FONT_HERSHEY_SIMPLEX, 0.6, cv::Scalar(0, 255, 0), 2);
        }
    }
...
void FrameProcessor::drawText(cv::Mat& image) {
    ...
    switch(mode) {
        ...
        case ProcessingMode::FACE_DETECT_MODE: modeText = "FACE DETECTION (Multi-threaded)"; break;
    }
    ...
    y += 20;
    cv::putText(image, "d-Draw z-Zoom r-Rotate f-Face Detection ESC-Exit", 
                cv::Point(10, y), cv::FONT_HERSHEY_SIMPLEX, 0.5, cv::Scalar(255, 255, 0), 1);
}
